...
public class CharacterController2D : MonoBehaviour {
	...
	// variables handle dashing logic
	private float dashAcceleration = .01f;
	private float dashMax = 3f;
	private float dashAccelerating = 1f;
	...
	void Update()
	{
		...
		{
			DoJump();
		} else if (canDoubleJump && CrossPlatformInputManager.GetButtonDown("Jump")){ // if can double jump & jump button pressed, then allow player to double jump
			DoJump();
			// disables double jump as you can only double jump once in the air
			canDoubleJump = false;
		}
		...
		// for dash ability, an accelerating motion, condition true if both dash button and movement button is pressed
		// very hacky way to create dashing effect and it's not perfect.. yet
		if(Input.GetButton("Dash") && Input.GetButton("Horizontal")){
			dashAccelerating += dashAcceleration;
			dashSpeed = Mathf.Min(dashAccelerating, dashMax);
			_rigidbody.velocity = new Vector2(_vx * moveSpeed * dashSpeed, _vy);
		}

		// sets dashAccelerating back down to starting dash speed whenever dashing is done
		if(Input.GetButtonUp("Dash") || _vx == 0){
			dashAccelerating = 1f;
		}

		// if moving up then don't collide with platform layer
		// this allows the player to jump up through things on the platform layer
		// ALSO, player can dropdown from platforms
		// NOTE: requires the platforms to be on a layer named "Platform"
		Physics2D.IgnoreLayerCollision(_playerLayer, _platformLayer, ((_vy > 0.0f) || (isGrounded && (Input.GetAxis("Vertical") < 0))));
	}
	...
	// Adds extra life if extra lives token is taken
	public void CollectLife(int amount){
		PlaySound(livesSFX); // sounds delayed, may want to edit clip to shorten this
	...
	// when Sparty lands on an enemy/ switch, jump
